...
const ConfessionCard: React.FC<ConfessionCardProps> = ({ confession, onUpdate }) => {
  ...
  if (confession.isReadOnce && !isRead) {
      return (
        <div className="bg-slate-800 border border-rose-500/50 rounded-lg p-4 flex flex-col items-center justify-center h-48 animate-pulse">
            <p className="text-rose-400 font-semibold mb-2">This confession can only be read once.</p>
            <button
  ...
  return (
    <div className="bg-slate-800 rounded-lg shadow-lg overflow-hidden transition-all duration-300 hover:shadow-rose-500/20 border border-slate-700">
      <div className="p-5">
        <div className="flex justify-between items-start text-xs text-slate-400 mb-3">
            <div>
                {confession.tone && <span className="font-semibold bg-slate-700 text-rose-300 px-2 py-1 rounded-full">{confession.tone} Tone</span>}
                {confession.postType === 'emoji' && <span className="font-semibold bg-slate-700 text-amber-300 px-2 py-1 rounded-full">Emoji Only</span>}
            </div>
            <span>{timeAgo(confession.createdAt)}</span>
        </div>
        
        <p className={`text-slate-200 whitespace-pre-wrap ${confession.postType === 'emoji' ? 'text-4xl' : 'text-base'}`}>
            {confession.content}
        </p>
      ...
      <div className="bg-slate-800/50 px-5 py-3 flex justify-between items-center text-sm text-slate-400 border-t border-slate-700">
          <div className="flex items-center gap-4">
            <button onClick={handleUpvote} className="flex items-center gap-1 hover:text-rose-400 transition-colors">

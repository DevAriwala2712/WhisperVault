...
const AdminPanel: React.FC<AdminPanelProps> = ({ confessions, onUpdate, onDelete, adminSettings, setAdminSettings }) => {
  ...
  const getStatusColor = (status: ConfessionStatus) => {
    switch(status) {
      ...
      case ConfessionStatus.Pending: return 'text-amber-400 bg-amber-900/50';
  ...
  return (
    <div className="bg-slate-800 p-6 rounded-lg shadow-xl border border-slate-700 space-y-6">
      <h2 className="text-2xl font-bold text-rose-400">Admin Moderation Panel</h2>
      
      <div className="bg-slate-900 p-4 rounded-md border border-slate-700">
        <h3 className="text-lg font-semibold mb-2">Settings</h3>
        <label className="flex items-center gap-4">
          <span className="text-slate-300">Auto-delete posts older than:</span>
          <input
            type="number"
            value={adminSettings.retentionDays}
            onChange={handleRetentionChange}
            className="w-24 bg-slate-700 border border-slate-600 rounded-md p-2 text-center"
          />
      ...
      <div>
        ...
        <div className="space-y-4">
          {filteredConfessions.length > 0 ? filteredConfessions.map(confession => (
            <div key={confession.id} className="bg-slate-900 p-4 rounded-md border border-slate-700">
              <div className="flex justify-between items-start">
                  <p className="text-slate-300 pr-4">{confession.content}</p>
                  <span className={`text-xs font-mono px-2 py-1 rounded-full whitespace-nowrap ${getStatusColor(confession.status)}`}>
                    {confession.status}
                  </span>
              </div>
              {confession.moderationReason && <p className="text-xs text-rose-300 mt-2 italic">AI Reason: {confession.moderationReason}</p>}
          ...
          )) : <p className="text-slate-500 text-center py-4">No confessions match this filter.</p>}
        </div>
      </div>
    </div>
  );
...
const TabButton: React.FC<{label: string, isActive: boolean, onClick: () => void}> = ({label, isActive, onClick}) => (
    <button onClick={onClick} className={`px-4 py-2 text-sm font-medium transition-colors ${isActive ? 'border-b-2 border-rose-500 text-rose-400' : 'text-slate-400 hover:text-white'}`}>
        {label}
    </button>
);

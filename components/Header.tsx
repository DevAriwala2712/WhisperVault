...
const Header: React.FC<HeaderProps> = ({ currentView, setView, userRole, onLogout }) => {
  return (
    <header className="bg-slate-900/50 backdrop-blur-sm sticky top-0 z-10 shadow-md">
      <nav className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          <div className="flex items-center">
            <h1 className="text-xl font-bold text-rose-400">WhisperVault</h1>
          </div>
          <div className="flex items-center space-x-2">
            ...
            <button
              onClick={onLogout}
              className="flex items-center gap-2 px-3 py-2 text-sm font-medium rounded-md transition-colors duration-200 text-slate-300 hover:bg-rose-800/80 hover:text-white"
              aria-label="Logout"
            >
              <LogoutIcon />
              <span>Logout</span>
